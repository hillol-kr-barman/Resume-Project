  ...
  return (
    <div>
      ...
      <main className="mx-auto max-w-6xl px-5 pb-20 pt-28 lg:px-6">
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-surface/95 p-5 sm:p-6">
        ...
        <section className="mt-8 grid w-full gap-3 rounded-3xl border border-white/10 bg-surface/70 p-4 text-left sm:grid-cols-3 sm:p-5">
          <div>
          ...
          <div className="mt-4 space-y-4">
            {employment.map((job) => (
              <article key={job.company}>
                ...
                <p className="mt-1 text-sm/7 font-medium text-accent">
                  {job.company} &bull; {job.location}
                </p>
                <ul className="mt-3 list-disc space-y-1.5 pl-5 text-sm/7 text-muted">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {education.map((item) => (
              <article key={item.institution} className="card-panel-tight">
                ...
                <p className="mt-2 text-sm/7 text-muted">{item.notes}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="card-panel mt-8">
          ...
          <div className="mt-4 space-y-3">
            {timeline.map((item) => (
              <div
                key={item.period}
                className="grid gap-2 border-l-2 border-accent/60 pl-4 md:grid-cols-[14rem_minmax(0,1fr)]"
              >
                <p className="text-sm/7 font-semibold text-accent">{item.period}</p>
                <p className="text-sm/7 text-muted">{item.event}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="card-panel mt-8">
          ...
          <a
            ...
            download
            className="mt-6 inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2.5 text-sm font-semibold text-black transition-shadow duration-300 hover:shadow-[0_0_22px_color-mix(in_srgb,var(--color-accent)_55%,transparent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
          >
